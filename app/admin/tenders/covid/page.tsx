...
export default function CovidTendersPage() {
  ...
  useEffect(() => {
    if (tenders.length > 0) {
      localStorage.setItem("covid_tenders", JSON.stringify(tenders));
    }
  }, [tenders]);
  ...
  return (
    <section className="p-6 bg-gray-50 min-h-screen">
      ...
      <div className="bg-white rounded-xl shadow-lg p-6 space-y-6">
        <h1 className="text-2xl font-bold text-gray-800">Covid-19 Tenders</h1>
              ...
              {filtered.slice(0, entriesPerPage).map((t, idx) => (
                <tr key={t.id} className="hover:bg-blue-50 transition-colors duration-200">
                  ...
                  <td className="px-4 py-3 flex gap-2 justify-center">
                    <Link
                      href={`/admin/tenders/covid/view/${t.id}`}
