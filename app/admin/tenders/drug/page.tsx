...
export default function DrugTendersPage() {
  ...
  // Load from localStorage (with default tender)
  useEffect(() => {
    const saved = localStorage.getItem("drug_tenders");
    if (saved) {
      setTenders(JSON.parse(saved));
    } else {
      const defaultTender: Tender = {
        id: 1,
        title: "SHORT TENDER - RATE CONTRACT FOR MEDICINES",
        reference: "JMC/NIT-21/DRUGS/36",
        published: "2021-12-10",
        start: "2021-12-18",
        end: "2021-12-27",
      };
      setTenders([defaultTender]);
      localStorage.setItem("drug_tenders", JSON.stringify([defaultTender]));
    }
  }, []);
  ...
  useEffect(() => {
    if (tenders.length > 0) {
      localStorage.setItem("drug_tenders", JSON.stringify(tenders));
    }
  }, [tenders]);
              ...
              {filtered.slice(0, entriesPerPage).map((t, idx) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 flex gap-2 justify-center">
                    <Link
                      href={`/admin/tenders/drug/view/${t.id}`}
